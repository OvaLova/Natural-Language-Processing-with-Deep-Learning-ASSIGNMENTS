...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # remember to slice off the last extra step → length=L
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.post_embed_cnn(X)[:, :, 1:]  # slicing off the first extra step → length=L
        # X = self.post_embed_cnn(X)[:, :, :-1]  # slicing off the last extra step → length=L
    ...
    # def generate_sent_masks(self, source_padded: torch.Tensor) -> torch.Tensor:
    #     """
    #     Generate sentence masks for encoder hidden states.

    #     @param source_padded (Tensor): Tensor of shape (b, src_len) containing token IDs,
    #                                    including <pad> tokens, where b = batch size, src_len = max source length

    #     @returns enc_masks (Tensor): Float mask tensor of shape (batch_size, src_len),
    #                                 where 1 = masked (padding), 0 = unmasked (real tokens).
    #     """
    #     pad_id = self.vocab.src['<pad>']
    #     enc_masks = (source_padded == pad_id).float()
    #     return enc_masks.to(self.device)
